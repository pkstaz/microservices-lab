...
[source,sh]
----
$ oc exec -c kafka -i production-ready-kafka-0 -- curl -s -X POST -w "\n" \
    -H "Accept:application/json" \
    -H "Content-Type:application/json" \
    http://debezium-connect-api:8083/connectors -d @- <<'EOF'
...
[source]
$ oc exec -i production-ready-kafka-0 -- curl -s -X GET "elasticsearch:9200/orders_with_event/_search?pretty"
